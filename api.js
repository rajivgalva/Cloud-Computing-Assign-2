...
var initDrink = {uri: 'https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i='};
...
var apiCaller = function (urlin, cb) {
  ...
  var drinkCaller = function (urlin, cb) {
    //use request to make the external http call to the JSON api
    var url = urlin.concat(parameter.drinkid);
    console.log (url);
    request({
      url: url,
      json: true
  ...
  var apiDrink = function(cb) {
    return drinkCaller(initDrink.uri, cb);
  //return apiCaller(parameter);
